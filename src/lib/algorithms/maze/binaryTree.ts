...
export const binaryTree = async (
  ...
  for (let r = 1; r < MAX_ROWS; r += 2) {
    for (let c = 1; c < MAX_COLS; c += 2) {
      if (r === MAX_ROWS - 2 && c === MAX_COLS - 2) continue;
      else if (r === MAX_ROWS - 2) await destroyWall(grid, r, c, 1, speed);
      else if (c === MAX_COLS - 2) await destroyWall(grid, r, c, 0, speed);
      else await destroyWall(grid, r, c, getRandInt(0, 2), speed);
    }
  }
  setIsDisabled(false);
};
